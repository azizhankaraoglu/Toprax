...
def register_extra_routes(api_router, db, current_user, is_admin):
    ...
    async def ai_disease_detect(body: DiseaseDetectReq, user=Depends(current_user)):
        """
        ...
        try:
            if provider == "openai":
                ...
                resp = _requests.post(
                    ...
                    json={
                        "contents": [{"parts": [
                            {"text": system_prompt + "\n\nBu bitki fotoğrafını analiz et."},
                ...
                raise HTTPException(500, f"Bilinmeyen AI sağlayıcısı: {provider}")
...
"""
    ...
    async def _call_ai_text(ai_cfg: dict, system_prompt: str, user_text: str) -> str:
        ...
        provider, api_key, model = ai_cfg.get("provider"), ai_cfg.get("api_key"), ai_cfg.get("model")
        if provider == "openai":
            resp = requests.post(
                "https://api.openai.com/v1/chat/completions",
                headers={"Authorization": f"Bearer {api_key}"},
            ...
            resp = requests.post(
                "https://api.anthropic.com/v1/messages",
                headers={"x-api-key": api_key, "anthropic-version": "2023-06-01"},
                json={"model": model or "claude-sonnet-4-6", "max_tokens": 500,
    ...
    async def ai_copilot(body: CopilotQuery, user=Depends(current_user)):
        """
        ...
        # AI'nin ürettiği filtreyi GÜVENLİ şekilde MongoDB sorgusuna çevir
        # (AI'nin ürettiği metni doğrudan sorguya sokmuyoruz — sadece
        # beklenen alan adlarını okuyup kendi query'mizi inşa ediyoruz).
        mongo_q: Dict[str, Any] = {}
        if filt_spec.get("region_name"):
            region = await db.regions.find_one({"name": filt_spec["region_name"]}, {"_id": 0, "id": 1})
            if region:
                mongo_q["region_id"] = region["id"]
        if filt_spec.get("village"):
            mongo_q["village"] = filt_spec["village"]
        if filt_spec.get("risk_level"):
            rl = filt_spec["risk_level"]
            mongo_q["risk_level"] = {"$in": rl if isinstance(rl, list) else [rl]}
        if filt_spec.get("soil_type"):
            mongo_q["soil_type"] = filt_spec["soil_type"]
        if filt_spec.get("irrigation"):
            mongo_q["irrigation"] = filt_spec["irrigation"]
        if "min_ndvi" in filt_spec or "max_ndvi" in filt_spec:
            ndvi_q = {}
            if "min_ndvi" in filt_spec: ndvi_q["$gte"] = float(filt_spec["min_ndvi"])
            if "max_ndvi" in filt_spec: ndvi_q["$lte"] = float(filt_spec["max_ndvi"])
            mongo_q["ndvi_latest"] = ndvi_q
        ...
        sort_dir = -1 if filt_spec.get("sort_dir") == "desc" else 1
        limit = min(int(filt_spec.get("limit", 20)), 200)

        results = await db.parcels.find(mongo_q, {"_id": 0}).sort(sort_field, sort_dir).limit(limit).to_list(limit)
    ...
    async def get_ndvi_data(parcel_id: str, user=Depends(current_user)):
        """
        Parselin son uydu görüntüsünden NDVI değeri.
        MOCK VERİ — gerçek Sentinel Hub entegrasyonu için API key gerekir.
        """
        parcel = await db.parcels.find_one({"id": parcel_id}, {"_id": 0})
        if not parcel:
            raise HTTPException(404, "Parsel bulunamadı")
        
        # Rastgele ama sabit (parsel_id'ye göre DETERMİNİSTİK).
        # NOT: Python'un yerleşik hash() fonksiyonu güvenlik amacıyla
        # process başına rastgele salt kullanır — yani sunucu her
        # yeniden başladığında NDVI değerleri değişirdi. Bunun yerine
        # zlib.crc32 ile süreçten bağımsız sabit bir hash üretiyoruz.
        # Ayrıca global `random` modülünü değil yerel bir Random
        # örneği kullanıyoruz — global seed değiştirmek, aynı anda
        # gelen başka isteklerin rastgele veri üretimini bozabilirdi.
        import zlib
        rnd = random.Random(zlib.crc32(parcel_id.encode()) % 10000)
        
        # 5 aylık NDVI zaman serisi (Mayıs-Eylül)
        time_series = []
        base_ndvi = rnd.uniform(0.4, 0.75)
        for month in range(5, 10):
            for day in [1, 15]:
                # NDVI önce artar, sonra azalır (büyüme eğrisi)
                if month <= 7:
        ...
        if latest["ndvi"] > 0.65:
            health = {"status": "iyi", "label": "Sağlıklı gelişim", "color": "#4ade80"}
        elif latest["ndvi"] > 0.45:
            health = {"status": "orta", "label": "İzlemeye değer", "color": "#fbbf24"}
        else:
            health = {"status": "kötü", "label": "Stres altında", "color": "#ef4444"}
        ...
        return {
            ...
            "data_source": "MOCK (Sentinel Hub aktivasyonu sözleşme sonrası)"
        }
    ...
    async def seed_extras():
        ...
        # server.py'deki /admin/seed ile aynı bootstrap mantığı: kimlik
        # doğrulanmamış (soğuk başlangıç) çağrılarda "default" tenant'a yazılır.
        reset_token = None
