...
export default function FarmerDetail() {
  ...
  const [fieldDefs, setFieldDefs] = useState([]);           // Sprint A1 — Form Yönetimi alan tanımları (farmers)
  const [docCount, setDocCount] = useState(0);              // IT-04 — Belgeler sekmesi rozeti
  ...
  useEffect(() => {
    api.get(`/farmers/${id}`).then((r) => {
      setData(r.data);
      pushRecentlyViewed({ module: "farmers", id, label: r.data.farmer.full_name, path: `/ciftciler/${id}` });
    });
    api.get("/field-definitions", { params: { module: "farmers" } }).then((r) =>
      setFieldDefs(r.data.filter((f) => f.is_active !== false).sort((a, b) => a.order - b.order))
    );
    api.get("/uploads", { params: { module: "farmers", entity_id: id } }).then((r) => setDocCount(r.data.length));
  }, [id]);

  function startEdit() {
    setEditForm({
      full_name: data.farmer.full_name, phone: data.farmer.phone, email: data.farmer.email || "",
      village: data.farmer.village, iban: data.farmer.iban || "", notes: data.farmer.notes || "",
      ...Object.fromEntries(fieldDefs.map((f) => [f.field_key, data.farmer[f.field_key] ?? ""])),
    });
    setSaveError("");
    setEditing(true);
  }
  ...
  async function saveEdit() {
    ...
    setSaveError("");
    try {
      await api.put(`/farmers/${id}`, editForm);
      const r = await api.get(`/farmers/${id}`);
      setData(r.data);
      setEditing(false);
    } catch (err) {
  ...
  const tabs = [
    { id: "genel", label: "Genel Bilgiler", icon: Info, count: fieldDefs.filter((f) => farmer[f.field_key] !== undefined && farmer[f.field_key] !== null && farmer[f.field_key] !== "").length },
    ...
    { id: "ziyaret", label: "Ziyaret Geçmişi", icon: History },
    { id: "iletisim", label: "İletişim", icon: MessageCircle },
    { id: "belgeler", label: "Belgeler", icon: Paperclip, count: docCount },
  ];

  return (
    <div className="p-8 max-w-[1600px]" data-testid="farmer-detail-page">
      <Breadcrumb items={[{ label: "Çiftçiler", to: "/ciftciler" }, { label: farmer.full_name }]} />
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className="font-display text-2xl mt-1 flex items-center gap-2">
              {farmer.full_name}
              <FavoriteButton module="farmers" entityId={id} label={farmer.full_name} />
            </div>
      ...
      {/* HIZLI İŞLEMLER — IT-13 (context-aware CRUD: farmer_id otomatik dolar, parsel seçilir) */}
      <div className="card p-5 mb-6">
        ...
        {parcels.length === 0 ? (
          <div className="text-sm text-[var(--text-dim)]">Sözleşme/görev/sezon eklemek için önce bu çiftçiye bir parsel tanımlanmalı.</div>
        ) : (
          <div className="flex flex-wrap gap-3">
            ...
            <QuickAddPanel
              ...
              onSubmit={async (v) => {
                await api.post("/operations/tasks", { ...v, scheduled_date: new Date(v.scheduled_date).toISOString() });
              }}
            />
            <QuickAddPanel
              title="Yeni Sezon"
              testId="quick-cycle-add"
              fields={[
                { name: "parcel_id", label: "Parsel", type: "select", required: true,
                  options: parcels.map((p) => ({ value: p.id, label: `${p.parcel_code} — ${p.name}` })) },
                { name: "year", label: "Yıl", type: "number", required: true, default: new Date().getFullYear() },
                { name: "season", label: "Sezon", default: "Ana Ürün" },
                { name: "crop", label: "Ürün", default: "Şeker Pancarı" },
              ]}
              onSubmit={async (v) => {
                await api.post("/production-cycles", { ...v, farmer_id: id, year: Number(v.year) });
              }}
            />
          </div>
        )}
      ...
      <div className="fade-in">
        {/* Genel Bilgiler — Sprint A1 dinamik alanlar (Form Yönetimi'nden) + IT-04 düzenleme modu */}
        {tab === "genel" && (
            ...
            <div className="flex items-center justify-end mb-3">
              {!editing ? (
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={saveEdit} disabled={saving} className="btn btn-primary text-xs" data-testid="farmer-edit-save">
              ...
              <div className="space-y-5">
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    <div>
                      <label className="text-xs text-[var(--text-dim)] mb-1 block">Ad Soyad *</label>
                      <input className="input" value={editForm.full_name ?? ""} required
                             onChange={(e) => setEditForm((f) => ({ ...f, full_name: e.target.value }))} />
                    </div>
                    <div>
                      <label className="text-xs text-[var(--text-dim)] mb-1 block">Telefon *</label>
                      <input className="input" value={editForm.phone ?? ""} required
                             onChange={(e) => setEditForm((f) => ({ ...f, phone: e.target.value }))} />
                    </div>
                    <div>
                      <label className="text-xs text-[var(--text-dim)] mb-1 block">E-posta</label>
                    ...
                    <div>
                      <label className="text-xs text-[var(--text-dim)] mb-1 block">IBAN</label>
                      <input className="input" value={editForm.iban ?? ""}
        ...
        {tab === "belgeler" && <DocumentsTab module="farmers" entityId={id} />}
        ...
        {/* Ziyaret Geçmişi — IT-23 */}
        {tab === "ziyaret" && <VisitHistory farmerId={farmer.id} />}

        {/* İletişim — IT-25 */}
        {tab === "iletisim" && <CommunicationTab contactType="farmer" contactId={farmer.id} contactName={farmer.full_name} />}
      </div>
    </div>
  );
}
