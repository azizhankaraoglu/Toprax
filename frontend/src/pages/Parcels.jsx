...
export default function Parcels() {
  ...
  function activateTool(t) {
    ...
    const msgs = {
      ...
      import: "Bir .geojson dosyası seçin.",
    };
    setToolMsg(msgs[t] || "");
  }
  ...
  async function submitNewParcel(values) {
    ...
    await api.post("/parcels", {
      ...
      region_id: farmers.find((f) => f.id === values.farmer_id)?.region_id,
      area_dekar: area,
  ...
  async function submitManualParcel(values) {
    ...
    await api.post("/parcels", {
      ...
      village: values.village,
      region_id: farmers.find((f) => f.id === values.farmer_id)?.region_id,
      area_dekar: Number(values.area_dekar),
      soil_type: values.soil_type,
      irrigation: values.irrigation,
      geometry: null,
    });
  ...
  const TOOLS = [
    ...
    { key: "import", icon: Upload, label: "GeoJSON İçe Aktar" },
  ];
    ...
    <div className="p-8 max-w-[1700px]" data-testid="parcels-page">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
        ...
        <div className="card p-4 overflow-y-auto scrollbar" style={{ maxHeight: 640 }}>
          ...
          {tool === "import" && (
            <div>
              <h3 className="font-display text-lg mb-3">GeoJSON İçe Aktar</h3>
