# AI-VIZYON-PLATFORMU-MIMARI.md — TABSİS Agricultural Intelligence Engine

> Bu doküman `AI-VIZYON-PLATFORMU-PROMPT.md`'nin çıktısıdır. CLAUDE.md,
> ROADMAP.md, ROADMAP-DETAY-TAM.md okunduktan sonra, TABSİS'in mevcut
...
> Bağımsız bir sistem değil, TABSİS'in bir admin modülüdür.
...
1. **Ölçek gerçek değil, varsayımsal:** Faz-1 hedefi 3000-5000 parsel —
   ...
   bu ölçekte devreye girmez. TABSİS zaten `parcels`/`admin_areas`'ta
...
5. **Kaçış yolu açık bırakıldı:** Faz 2+'da (bkz. Bölüm 15) AI Engine
   ...
   veritabanı bu noktada değiştirilebilir — geri kalan TABSİS'e dokunmadan
   (mikroservis izolasyonu zaten bu kaçışı sağlıyor). Bugün bunu seçmemek,
   yarın seçememek anlamına gelmiyor.

**Sonuç:** AI Engine, TABSİS'in geri kalanıyla AYNI MongoDB örneğinde
kendi koleksiyonlarını kullanır (ayrı bir `tabsis_ai` mantıksal veritabanı
...
```
                         ...
                         │  örneğine bağlanır (tabsis_ai veritabanı)   │
                         │                                             │
                         │  ┌───────────────────────────────────────┐  │
...
├── server.py                           # FastAPI app, /ai prefix'i, TABSİS api_router'a mount edilir (Faz 1)
...
- Bu paket **kendi kimlik doğrulamasını, kendi RBAC'ını, kendi audit
  ...
  mekanizmasını YAZMAZ.** Her biri için TABSİS'in mevcut modülünü
...
Mantıksal veritabanı adı: `tabsis_ai` (aynı Mongo örneği, ayrı DB — hem
...
ai_knowledge_records         { id, dataset_id, tenant_id,
                               ...
                               # TABSİS varlık bağlantıları (opsiyonel, prompt'un istediği eklenti):
...
ai_audit_extension             # AYRI koleksiyon YOK — TABSİS'in mevcut audit_logs'u kullanılır,
...
ai_datasets ──1:N── ai_knowledge_records ──N:1── ai_taxonomy (labels üzerinden)
                         ...
                         ▼
        TABSİS: parcels / production_cycles / farmers
                         │
                         ▼
...
uğratmaz** — TABSİS'in ledger.py'deki "silinmezlik" felsefesiyle aynı
...
ailesi en olgun/hızlı seçenek olduğu için cazip, ama TABSİS ticari
...
Faz 1 — Tek Sunucu (mevcut hedef)
  [ Mongo | FastAPI (TABSİS) | FastAPI (AI Engine, aynı process'e mount) | React | Nginx ]
  Job worker: aynı process içinde asyncio background task.

Faz 2 — Ayrı AI Sunucusu
  [ Sunucu A: Mongo | TABSİS API | React | Nginx ]
  [ Sunucu B: AI Engine (ayrı container/VM), AYNI Mongo'ya uzaktan bağlanır ]
  TABSİS → AI Engine çağrıları artık network üzerinden (mevcut /api/ai/*
  path'i DEĞİŞMEZ) — bir "Service Registry" kaydı (yeni bir koleksiyon,
  `service_registry`: {service_name, base_url, health_check_path}) TABSİS'e
...
Faz 4 — Dağıtık AI Cluster
  ...
  [ Mongo (replika seti — bu noktada zaten TABSİS'in genel HA ihtiyacıyla örtüşür) ]
```
...
gereksinimini TABSİS'in kendi diliyle (Service Registry + değişmeyen
