# PROMPT — TOPRAX Agricultural Intelligence & AI Vision Platform (Admin Module)
...
> başına yeterli değil — TOPRAX'in mevcut mimari kurallarını (Config Service,
...
üstleniyorsun. TOPRAX platformunun **Agricultural Intelligence Engine**'ini
...
TOPRAX'in içine, mevcut kurallarına uyarak entegre olacak bir **admin
modülü**. Aşağıdaki "TOPRAX Entegrasyon Zorunlulukları" bölümü, geri kalan
...
TOPRAX (Tarımsal Bilgi Sistemi) sadece bir GIS uygulaması veya Çiftçi Kayıt
...
**Eklenen alt ilke — Tenant Bazlı Maliyet İzolasyonu:** TOPRAX çok kiracılı
...
**Eklenen not — mevcut TOPRAX altyapısıyla çelişki kontrolü:** ROADMAP.md'nin
...
TOPRAX'in ana veritabanı **MongoDB**'dir (PostgreSQL/PostGIS değil). Bu
prompt'taki PostgreSQL/PostGIS/Redis/RabbitMQ talebi, TOPRAX'in geri kalanıyla
...
RabbitMQ) kullanır, TOPRAX ana sistemiyle sadece API/event üzerinden konuşur
...
- **Uydu görüntüsü:** TOPRAX'te zaten IT-17'de bir Uydu Provider Soyutlaması
...
- **Mobil görüntü:** TOPRAX'in mobil rol matrisinde (IT-35) Saha Personeli
...
**Eklenen — TOPRAX varlıklarıyla ilişki (orijinalde eksikti):** Her
Knowledge Library kaydı, mümkün olduğunda TOPRAX'in mevcut varlıklarına
...
**Eklenen kural:** Audit log kendi mekanizmasını icat etmez, TOPRAX'in ortak
...
**Eklenen — lisans kontrolü ticari bağlama bağlanmalı:** TOPRAX ticari bir
...
**Eklenen — mevcut Uzman Desteği modülüyle çakışmayı önleme:** TOPRAX'te
...
tespit edildi, risk oluştu) TOPRAX'in `platform/events.py` event bus'ına
...
**Eklenen — TOPRAX kurallarıyla zorunlu uyum:**
- Görüntü/dosya depolama TOPRAX'in `storage.py` soyutlamasını kullanır
  (IT-04), yeni bir depolama katmanı icat edilmez.
- Tüm API'ler TOPRAX'in Standard API sözleşmesine (Create/Update/
...
- Knowledge Search, kendi arama motorunu yazmaz — TOPRAX Query Engine'in
...
TOPRAX'in zaten planlı **Service Registry** (IT-32/33) prensibiyle
...
TOPRAX'in kendi diliyle karşılar.
